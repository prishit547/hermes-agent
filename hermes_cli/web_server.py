...
# Audio MIME→extension mapping, upload limits, and the decode/transcribe/
# synthesize helpers now live in tools.audio_endpoints_core so this dashboard
# server and the api_server share one implementation (see /api/audio/* below).
...
async def transcribe_audio_upload(payload: AudioTranscriptionRequest):
    # Shared decode + transcribe logic lives in tools.audio_endpoints_core so
    # this dashboard endpoint and the api_server's /v1/audio/transcriptions
    # stay in sync (same validation, provider dispatch, temp-file handling).
    ...
    try:
        audio_bytes, mime_type = decode_audio_data_url(
            payload.data_url, payload.mime_type
        )
        loop = asyncio.get_running_loop()
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    """
    # Shared synthesis logic lives in tools.audio_endpoints_core so this
    # dashboard endpoint and the api_server's /v1/audio/speech stay in sync.
    from tools.audio_endpoints_core import (
        AudioRequestError,
        speech_data_url,
        synthesize_speech,
    ...
    try:
        ...
        audio_bytes, mime_type, provider = await loop.run_in_executor(
            None, synthesize_speech, payload.text
        )
    except AudioRequestError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail)
    except Exception as exc:
